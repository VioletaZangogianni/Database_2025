WITH band_perf (band_id, performance_id, music_event_id) AS (SELECT band_id, performance_id, music_event_id FROM band INNER JOIN performer NATURAL JOIN performance ON band.band_performer_id = performer.performer_id),
     ...
     g_artists AS (SELECT artist_id, genre_desc FROM art_g_year WHERE genre_desc = 'Rock')
SELECT DISTINCT artist_id AS id, artist_name, case
	WHEN 2018 IN (SELECT fest_year FROM (SELECT artist_id, fest_year FROM art_g_year WHERE artist_id = id) AS T) THEN 'YES'
    ELSE 'NO'
